...
def normalize(points):
    ...
    dst = np.sqrt((np.square(points[0] - mean_x)) + np.square(points[1] - mean_y))
    mean_dst = np.mean(dst)
    
    s = np.sqrt(2)/mean_dst
    tx = -s*mean_x
    ty = -s*mean_y
...
def Inliers(H, points1, points2, th):
    ...
    trans_p1 = H@points1
    trans_p2 = np.linalg.inv(H)@points2
    ...
    dist = np.sum(np.square(points2 - trans_p1), axis=0) + np.sum(np.square(trans_p2 - points1), axis=0)
